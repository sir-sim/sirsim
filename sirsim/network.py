...
from sirsim.connection import Connection
from sirsim.ensemble import Ensemble
from sirsim.exceptions import (
    ConfigError, NetworkContextError, NotAddedToNetworkWarning, ReadonlyError)
from sirsim.node import Node
from sirsim.params import IntParam, StringParam
from sirsim.probe import Probe
from sirsim.utils.compat import iteritems
...
class Network(object):
    ...
    For example::
        ...
        network = sirsim.Network()
        with network:
            with sirsim.Network(label="Vision"):
                v1 = sirsim.Ensemble(sirsim.LIF(100), dimensions=2)
            with sirsim.Network(label="Motor"):
                sma = sirsim.Ensemble(sirsim.LIF(100), dimensions=2)
            sirsim.Connection(v1, sma)

    To reuse a group of related objects, you can create a new subclass
    of ``Network``, and add objects in the ``__init__`` method.
    For example::

        class OcularDominance(sirsim.Network):
            def __init__(self):
                self.column = sirsim.Ensemble(sirsim.LIF(100), dimensions=2)

        network = sirsim.Network()
        with network:
            left_eye = OcularDominance()
            right_eye = OcularDominance()
            sirsim.Connection(left_eye.column, right_eye.column)
    ...
    @property
    def n_agents(self):
        """(int) Number of agents in this network, including subnetworks."""
        return sum(ens.n_agents for ens in self.all_ensembles)
