...
import sirsim.utils.least_squares_solvers as lstsq
...
from sirsim.utils.compat import range, with_metaclass
from sirsim.utils.least_squares_solvers import (
    format_system, rmses, LeastSquaresSolverParam)
from sirsim.utils.magic import DocstringInheritor
...
class Solver(with_metaclass(DocstringInheritor, FrozenObject)):
    ...
    def __call__(self, A, Y, rng=np.random, E=None):
        ...
        Parameters
        ----------
        A : (n_eval_points, n_agents) array_like
            Matrix of the agents' activities at the evaluation points
        ...
        E : (dimensions, post.n_agents) array_like, optional (Default: None)
        ...
        Returns
        -------
        X : (n_agents, dimensions) or (n_agents, post.n_agents) ndarray
            (n_agents, dimensions) array of decoders (if ``solver.weights``
            is False) or (n_agents, post.n_agents) array of weights
    ...
    def mul_encoders(self, Y, E, copy=False):
        ...
        E : (dimensions, n_agents) array_like or None
...
class LstsqL2nz(_LstsqL2Solver):
    ...
    def __call__(self, A, Y, rng=np.random, E=None):
        ...
        # sigma == 0 means the agent is never active, so won't be used, but
...
class NoSolver(Solver):
    ...
    Parameters
    ----------
    values : (n_agents, n_weights) array_like, optional (Default: None)
        ...
        ``n_weights`` is the number of agents in the post ensemble.
    ...
    def __call__(self, A, Y, rng=None, E=None):
        if self.values is None:
            n_agents = np.asarray(A).shape[1]
            if E is not None:
                return np.zeros((n_agents, np.asarray(E).shape[1])), {}
            else:
                return np.zeros((n_agents, np.asarray(Y).shape[1])), {}
