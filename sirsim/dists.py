...
from sirsim.params import (BoolParam, IntParam, NdarrayParam, NumberParam,
...
class CosineSimilarity(SubvectorLength):
    ...
    and ``v``. In other words, a agent with intercept ``ppf(1 - p)`` will
...
def get_samples(dist_or_samples, n, d=None, rng=np.random):
    ...
    >>> mean([1, 2, 3, 4])
    2.5
    >>> mean(sirsim.dists.Gaussian(0, 1))
