...
def rates_isi(t, spikes, midpoint=False, interp='zero'):
    ...
    spikes : (M, N) array_like
        The raw spike data from N agents.
    ...
    rates : (M, N) array_like
        The estimated agent firing rates.
    """
...
def rates_kernel(t, spikes, kind='gauss', tau=0.04):
    ...
    tau : float
        ...
        firing rate of the agents, with a longer tau preferred for lower
...
def settled_firingrate(step_math, J, states,
    ...
    account some characteristics of spiking agents. We start
    by simulating the agents for a short amount of time, to let any
    initial transients settle. Then, we run the agents for a second
    and find the average (which should approximate the firing rate).

    Parameters
    ---------
    step_math : function
        the step function of the agent type
    J : ndarray
