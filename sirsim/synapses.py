...
from sirsim.base import Process
from sirsim.exceptions import ValidationError
from sirsim.params import (BoolParam, NdarrayParam, NumberParam, Parameter,
                          Unconfigurable)
from sirsim.utils.compat import is_number
from sirsim.utils.filter_design import cont2discrete
from sirsim.utils.numpy import as_shape
...
class Synapse(Process):
    ...
    to allow more or less current to flow into the agent.

    In Sirsim, the implementation of a synapse is as a specific case of a
    ...
    that will be induced in the postsynaptic agent.

    Synapses also contain the `.Synapse.filt` and `.Synapse.filtfilt` methods,
    which make it easy to use Sirsim's synapse models outside of Sirsim
    simulations.
...
class LinearFilter(Synapse):
    ...
    def evaluate(self, frequencies):
        ...
        Using the ``evaluate`` function to make a Bode plot::
            ...
            synapse = sirsim.synapses.LinearFilter([1], [0.02, 1])
...
def filt(signal, synapse, dt, axis=0, x0=None, copy=True):
    ...
    .. note:: Deprecated in Sirsim 2.1.0.
