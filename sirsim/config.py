...
from sirsim.exceptions import ConfigError, ValidationError
from sirsim.params import Default, is_param, iter_params
from sirsim.rc import rc
from sirsim.utils.compat import itervalues, reraise
from sirsim.utils.threading import ThreadLocalStack
...
class Config(object):
    ...
    *configures
    ...
    To configure defaults on a network::
        ...
        net = sirsim.Network()
        net.config[sirsim.Ensemble].radius = 1.5
        with net:
            ens = sirsim.Ensemble(10, 1)
        ens.radius == 1.5  # True

    To add a new parameter to a Sirsim object::

        net.config[sirsim.Ensemble].set_param(
            'location', sirsim.params.Parameter('location'))
        net.config[ens].location = 'cortex'

    To group together a set of parameters::

        gaba = sirsim.Config(sirsim.Connection)
        gaba[sirsim.Connection].synapse = sirsim.Lowpass(0.008)
        with net, gaba:
            conn = sirsim.Connection(ens, ens)
        conn.synapse == sirsim.Lowpass(0.008)  # True

    To configure a new type of object::

        class SynapseInfo(object):
            label = sirsim.params.StringParam('label')
    ...
    @staticmethod
    def all_defaults(sirsim_cls=None):
        """Look up all of the default values in the current context.

        Parameters
        ----------
        sirsim_cls : class, optional
        ...
        if sirsim_cls is None:
            ...
            lines.append("Current defaults for %s:" % sirsim_cls.__name__)
            for attr in dir(sirsim_cls):
                desc = getattr(sirsim_cls, attr)
                if is_param(desc) and desc.configurable:
                    val = Config.default(sirsim_cls, attr)
                    lines.append("  %s: %s" % (attr, val))
        return "\n".join(lines)

    @staticmethod
    def default(sirsim_cls, param):
        ...
        for config in reversed(Config.context):
            ...
            if sirsim_cls in config.params and config[sirsim_cls] in desc:
                return getattr(config[sirsim_cls], param)
