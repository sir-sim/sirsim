...
import sirsim
from sirsim.config import SupportDefaultsMixin
from sirsim.exceptions import NotAddedToNetworkWarning, ValidationError
...
from sirsim.utils.compat import is_integer, iteritems, range, with_metaclass
from sirsim.utils.numpy import as_shape, maxint
...
class NetworkMember(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        if add_to_container:
            sirsim.Network.add(inst)
        inst._initialized = True
        return inst


class SirsimObject(with_metaclass(NetworkMember, SupportDefaultsMixin)):
    """A base class for Sirsim objects.
    ...
    # This is needed for pickling and copying of Sirsim objects when the
    ...
    def __init__(self, label, seed):
        super(SirsimObject, self).__init__()
    ...
    def __setattr__(self, name, val):
        ...
        if initialized and not hasattr(self, name):
            warnings.warn(
                ...
                SyntaxWarning)
        super(SirsimObject, self).__setattr__(name, val)
...
class ObjView(object):
    ...
    This is used by the __getitem__ of Agents, Node, and Ensemble, in order
...
class SirsimObjectParam(Parameter):
    def __init__(self, name, optional=False, readonly=True,
        ...
        super(SirsimObjectParam, self).__init__(
