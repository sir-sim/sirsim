...
from sirsim.dists import DistributionParam, Gaussian
...
from sirsim.synapses import LinearFilter, Lowpass, SynapseParam
from sirsim.utils.compat import is_number, iteritems, itervalues
...
class Piecewise(Process):
    ...
    >>> from sirsim.processes import Piecewise
    >>> process = Piecewise({0.5: 1, 0.75: -1, 1: 0})
    >>> with sirsim.Network() as model:
    ...     u = sirsim.Node(process, size_out=process.default_size_out)
    ...     up = sirsim.Probe(u)
    >>> with sirsim.Simulator(model) as sim:
    ...     sim.run(1.5)
    >>> f = sim.data[up]
    >>> t = sim.trange()
    >>> f[t == 0.2]
