...
from sirsim.dists import DistOrArrayParam
from sirsim.ensemble import Ensemble, Agents
from sirsim.exceptions import ValidationError
from sirsim.learning_rules import LearningRuleType, LearningRuleTypeParam
from sirsim.agents import Direct
from sirsim.node import Node
from sirsim.params import (Default, Unconfigurable, ObsoleteParam,
                          BoolParam, FunctionInfo, Parameter)
from sirsim.solvers import LstsqL2, SolverParam
from sirsim.synapses import Lowpass, SynapseParam
from sirsim.transforms import Dense, Transform
from sirsim.utils.compat import is_array_like, is_iterable, iteritems
from sirsim.utils.functions import function_name
from sirsim.utils.stdlib import checked_call

logger = logging.getLogger(__name__)


class PrePostParam(SirsimObjectParam):
...
class ConnectionLearningRuleTypeParam(LearningRuleTypeParam):
    ...
    def check_rule(self, conn, rule):
        ...
        if rule.modifies in ('decoders', 'weights'):
            # pre object must be an agent
            if not isinstance(pre, (Ensemble, Agents)):
                raise ValidationError(
                    "'pre' must be of type 'Ensemble' or 'Agents' for "
                    "learning rule '%s' (got type %r)" % (
                        rule, type(pre).__name__),
                    attr=self.name, obj=conn)
            if (isinstance(pre, Ensemble)
                    and isinstance(pre.agent_type, Direct)):
                raise ValidationError(
                    "'pre' cannot have agent type 'Direct'. Connections from "
        ...
        if rule.modifies == 'encoders':
            ...
            if not isinstance(conn.post_obj, (Ensemble, Agents, Node)):
                raise ValidationError(
                    "'post' must be of type 'Ensemble', 'Agents' or 'Node' "
        ...
        if rule.modifies == 'weights':
            ...
            if conn.is_decoded:
                raise ValidationError(
                    ...
                    "connecting between two Agents objects." % rule,
                    attr=self.name, obj=conn)

            # transform matrix must be 2D
            pre_size = (
                pre.n_agents if isinstance(pre, Ensemble)
                else conn.pre.size_out)
            post_size = conn.post.size_in
            if (not conn.solver.weights
                    and conn.transform.shape != (post_size, pre_size)):
                raise ValidationError(
                    "Transform must be 2D array with shape post_agents x "
                    "pre_agents (%d, %d)" % (pre_size, post_size),
                    attr=self.name, obj=conn)
...
class Connection(SirsimObject):
    ...
    Almost any Sirsim object can act as the pre or post side of a connection.
    ...
    ``size_in=1``, we could not create the following connection::
        ...
        sirsim.Connection(node, ensemble)

    But, we could create either of these two connections::

        sirsim.Connection(node[0], ensemble)
        sirsim.Connection(node[1], ensemble)

    Parameters
    ----------
    pre : Ensemble or Agents or Node
        The source Sirsim object for the connection.
    post : Ensemble or Agents or Node or Probe
        The destination object for the connection.

    synapse : Synapse or None, optional \
              (Default: ``sirsim.synapses.Lowpass(tau=0.005)``)
        Synapse model to use for filtering (see `~sirsim.synapses.Synapse`).
    ...
    solver : Solver, optional (Default: ``sirsim.solvers.LstsqL2()``)
        Solver instance to compute decoders or weights
        (see `~sirsim.solvers.Solver`). If ``solver.weights`` is True, a full
    ...
    post : Ensemble or Agents or Node or Probe or ObjView
        The given post object.
    post_obj : Ensemble or Agents or Node or Probe
    ...
    pre : Ensemble or Agents or Node or ObjView
        The given pre object.
    pre_obj : Ensemble or Agents or Node
    ...
    modulatory = ObsoleteParam(
        ...
        url="https://github.com/sirsim/sirsim/issues/632#issuecomment-71663849")
    ...
    def is_decoded(self):
        return not (self.solver.weights or (
            isinstance(self.pre_obj, Agents)
            and isinstance(self.post_obj, Agents)))
    ...
    def size_in(self):
        ...
        if size_in == 'pre':
            ...
            return (conn.pre_obj.ensemble.size_out
                    if isinstance(conn.pre_obj, Agents) else conn.size_in)
        elif size_in == 'post_state':
            return (conn.post_obj.ensemble.size_in
                    if isinstance(conn.post_obj, Agents) else conn.size_out)
        else:
